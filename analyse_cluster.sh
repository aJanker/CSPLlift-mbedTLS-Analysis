...
flags=" --bdd \
	...
	-I $path/include \
	-I $path/library \
  	--adjustLines --interface --serializeAST --writePI --recordTiming --parserstatistics --lexNoStdout "


filesToProcess|while read i; do
         echo "Analysing $path/$i"
         echo "With settings: $flags"
         # $path/../TypeChef/typechef.sh $path/$i $flags
	sbatch -p hydra  -A hydra -n 1 -c 2 --time=06:00:00  --mem_bind=local --output=/dev/null --error=/dev/null $path/../TypeChef/typechef.sh $path/$i $flags
	done
